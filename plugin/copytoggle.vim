...
function s:On()
    ...
    for option in s:save
        execute 'silent! let option[2] = &'.option[0]
        execute 'silent! let &'.option[0].' = option[1]'
    endfor
...
function s:Off()
    ...
    for option in s:save
        execute 'silent! let untouched = &'.option[0].' == option[1]'
        if untouched
            execute 'silent! let &'.option[0].' = option[2]'
        endif
    endfor
    if g:copytoggle_notify
        echo ''
